...
### 2. **cpp (Context Provider Protocol)** (Rust, Python, JSON-RPC 2.0, WebSockets)
*Open protocol and runtime for feeding structured, budget-managed, graph-aware context to AI coding agents.*
*   Designed and engineered an open specification and high-performance **Rust** runtime for Context Provider Protocol (CPP), solving prompt inflation and unstructured context delivery in AI agent workflows.
*   Implemented a deterministic source-side **Budget Solver** enforcing strict `maxBytes` and `maxObjects` constraints, achieving **99.8% context volume reduction** (232 bytes vs 122KB raw scan) and **<2.1ms in-memory resolution latency**.
*   Built standardized **JSON-RPC 2.0** and WebSocket wire transports supporting a 3-level MIME taxonomy (`application/cpp.document.file`), graph relations (`Contains`, `References`), and real-time push streams.
*   Developed a **Python SDK**, an **MCP Bridge** (`cpp_query`, `cpp_resolve`, `cpp_capabilities`), and 6 data providers (Git, Filesystem, Datetime, GitHub, Jira, Slack).

### 3. **OwnedC** (C, Python, CMake)
...
### 5. **qubo-space-routing & SPICE-ns-Project** (C++, Python, LaTeX)
...
### 6. **ninai** (Electron, React 19, TypeScript 5.9, Vite 7)
...
### 9. **cnl-codetainers** (Docker, Go, Shell)
